...
The YAML sets the title of the document (My matriks), the author ("Me, myself, and I"), the date, and other settings concerning the document. For instance, this YAML will generate a PDF document with a **T**able **o**f **C**ontents. 
...
A warning has appeared since some type of distractors cannot be generated given the type of matriks and/or the rules used for its generation. In this case, since it is a single-laye matriks it is not possible to generate the Incomplete Correlate-Incomplete distractors, which is also crossed with a thick black cross. The argument `main` set `TRUE` means that the names of the distractors are plotted as well. 

The user can also specific a selection of the response options to associate to the matriks. The selection should be a character vector and it must be specified through th argument `distractors` of the `draw()` function. 
